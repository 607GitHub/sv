...
class MinibatchCKA(tf.keras.metrics.Metric):
    ...
    def __init__(self,
        ...
        if across_models:
            self.hsic_accumulator_model1 = self.add_weight(
                name='hsic_accumulator_model1',
                shape=(num_layers,),
                initializer=tf.keras.initializers.zeros,
                dtype=dtype)
            self.hsic_accumulator_model2 = self.add_weight(
                name='hsic_accumulator_model2',
                shape=(num_layers2,),
                initializer=tf.keras.initializers.zeros,
                dtype=dtype)
    ...
    def result(self):  # Modified by David Bikker to work around a tensorflow issue on the cluster computer
        import torch
        mean_hsic = torch.Tensor(tf.convert_to_tensor(
            self.hsic_accumulator).numpy())  # (num_layers, num_layers2)
